...
export default class Physics
{
    ...
    setMaterials()
    {
       ...
       // this.materials.items.wheel = new CANNON.Material('wheelMaterial')
        ...
        this.materials.contacts.floorDummy = new CANNON.ContactMaterial(this.materials.items.floor, this.materials.items.dummy, { friction: 0.05, restitution: 0.3, contactEquationStiffness: 100000 })
    ...
    setCar()
    {
        ...
        this.car.create = () =>
        {
            ...
            for(const _wheelInfos of this.car.vehicle.wheelInfos)
            {
                ...
                body.type = CANNON.Body.kinematic         // used to be kinematic, but can't interact with static objects. 
    ...
    addObjectFromThree(_options)
    {
        ...
        for(let i = 0; i < _options.meshes.length; i++)
        {
            ...
            else if(shape)
            {
                ...
                {
                    let halfExtents = new CANNON.Vec3(mesh.scale.x * 0.5, mesh.scale.y * 0.5, mesh.scale.z * 0.5)
                    ///////
                    //////
                    /////
                    ////// ok so this took fucking forever but I got it
                    ...
                    if(_options.scale){
                        ...
                        if(_options.added){                 //if the mesh is added
                            halfExtents = new CANNON.Vec3(((mesh.scale.x * _options.scale.x)  ), (mesh.scale.y * _options.scale.y), (mesh.scale.z * _options.scale.z))
                        }
                        else{
                            halfExtents = new CANNON.Vec3(((mesh.scale.x * _options.scale.x) * .5), (mesh.scale.y * _options.scale.y) *.5, (mesh.scale.z * _options.scale.z) *.5)

                        }
                    
                    }
                    else{
                        halfExtents = new CANNON.Vec3((mesh.scale.x) * 0.5, mesh.scale.y * 0.5, mesh.scale.z * 0.5)
                    }
                ...
                // Save
                // it appears that anything that isn't directly created as a 
        ...
        this.time.on('tick', () =>
        {
            collision.model.container.position.set(collision.body.position.x, collision.body.position.y, collision.body.position.z)             //sets position
            collision.model.container.quaternion.set(collision.body.quaternion.x, collision.body.quaternion.y, collision.body.quaternion.z, collision.body.quaternion.w)    //sets rotation
